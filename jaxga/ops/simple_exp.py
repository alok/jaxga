...
def get_mv_simple_exp(a_blade_indices, signature):
    ...
    mv_select_scalar, scalar_indices = get_mv_select(a_sq_indices, is_scalar_index)
    ...
    def _values_mv_simple_exp(a_values):
        ...
        out_scalar = jnp.where(
            a_sq_sqrt < 0,
            jnp.cos(a_sq_sqrt),
            jnp.where(a_sq_sqrt > 0, jnp.cosh(a_sq_sqrt), jnp.ones_like(a_sq_sqrt)),
        )
        ...
        out_blade = jnp.where(
            ...
            jnp.where(
                a_sq_sqrt > 0, a_values / a_sq_sqrt * jnp.sinh(a_sq_sqrt), a_values
            ),
        )
